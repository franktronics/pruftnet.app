...
TEST_CASE("NetworkSniffer can capture packets", "[network_scanner]") {
  SECTION("Basic packet capture test") {
    ...
    auto callback = [&packet_count,
                     &total_bytes_captured](const RawPacket &packet) {
      packet_count++;
      total_bytes_captured += packet.length;
      std::cout << "Packet #" << packet_count.load() << ": " << std::endl
    ...
    if (!started) {
      std::cout
          << "Warning: Could not start packet capture on interface "
    ...
    const size_t pcap_overhead =
        PCAP_GLOBAL_HEADER_SIZE + (total_packets * PCAP_PACKET_HEADER_SIZE);
    ...
    if (total_data_bytes < 1024) {
      std::cout << "Raw packet data size: " << total_data_bytes << " bytes"
                << std::endl;
    } else if (total_data_bytes < 1024 * 1024) {
      std::cout << "Raw packet data size: " << std::fixed
                << std::setprecision(2) << (total_data_bytes / 1024.0) << " KB"
                << std::endl;
    } else {
      std::cout << "Raw packet data size: " << std::fixed
                << std::setprecision(2)
    ...
    if (total_pcap_size < 1024) {
      std::cout << "PCAP file size would be: " << total_pcap_size << " bytes"
                << std::endl;
    } else if (total_pcap_size < 1024 * 1024) {
      std::cout << "PCAP file size would be: " << std::fixed
                << std::setprecision(2) << (total_pcap_size / 1024.0) << " KB"
                << std::endl;
    } else {
      std::cout << "PCAP file size would be: " << std::fixed
                << std::setprecision(2) << (total_pcap_size / (1024.0 * 1024.0))
                << " MB" << std::endl;
    }

    std::cout << "PCAP overhead: " << pcap_overhead << " bytes (" << std::fixed
              << std::setprecision(1)
              << (total_pcap_size > 0
                      ? (pcap_overhead * 100.0 / total_pcap_size)
                      : 0.0)
              << "%)" << std::endl;
    std::cout << "=========================================" << std::endl;
