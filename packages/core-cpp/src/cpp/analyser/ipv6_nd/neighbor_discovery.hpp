...
protected:
  ...
  bool sendPacket(const uint8_t* packet, size_t length, const std::string& interface_name);
  std::array<uint8_t, 16> parseIPv6Address(const std::string& ipv6_str);
  uint16_t calculateICMPv6Checksum(const std::array<uint8_t, MAX_PACKET_SIZE>& packet, size_t ipv6_offset,
