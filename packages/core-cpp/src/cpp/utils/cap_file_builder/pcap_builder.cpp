...
PcapBuilder::PcapBuilder(const std::string& filename)
...
PcapPacketHeader PcapBuilder::createPacketHeader(const RawPacket& packet) {
    ...
    header.original_length = packet.original_length > 0 ? 
        static_cast<uint32_t>(packet.original_length) : 
        static_cast<uint32_t>(packet.length);
