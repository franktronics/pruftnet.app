...
void PacketParser::parseProtocol(const RawPacket& raw_packet, ParsedPacket& parsed, ProtocolId protocol_id,
                                 size_t offset_bytes) {
  ...
  for (uint8_t i = 0; i < def->field_count && i < MAX_FIELDS_PER_PROTOCOL; ++i) {
    const FieldDefinition& field_def = def->fields[i];
    FieldEntry& field = entry.fields[i];

    field.field_id = field_def.field_id;
    field.byte_offset = static_cast<uint16_t>(offset_bytes + (field_def.bit_offset / 8));
    field.byte_length = static_cast<uint8_t>((field_def.bit_length + 7) / 8);
  }

  parsed.protocol_count++;

  ProtocolId next_protocol = determineNextProtocol(raw_packet, def, offset_bytes);
...
ProtocolId PacketParser::determineNextProtocol(const RawPacket& raw_packet, const ProtocolDefinition* def,
  ...
  for (uint8_t i = 0; i < def->field_count; ++i) {
    if (def->fields[i].field_id == def->next_protocol_field_id) {
      uint32_t value =
          extractFieldValue(raw_packet, header_offset_bytes, def->fields[i].bit_offset, def->fields[i].bit_length);
...
uint32_t PacketParser::extractFieldValue(const RawPacket& raw_packet, size_t header_offset_bytes, uint16_t bit_offset,
                                         uint16_t bit_length) {
  ...
  for (size_t i = 0; i < bytes_needed && (byte_offset + i) < raw_packet.length; ++i) {
