...
std::array<uint8_t, MAX_PACKET_SIZE> createEthernetIPv4TCPPacket() {
    ...
    packet[3] = 0xDD; packet[4] = 0xEE; packet[5] = 0xFF;
    ...
    packet[16] = 0x00; packet[17] = 0x34;  // Total Length (52 bytes)
    packet[18] = 0x12; packet[19] = 0x34;  // Identification
    packet[20] = 0x40; packet[21] = 0x00;  // Flags (010) + Fragment Offset (0)
    packet[22] = 0x40;  // TTL (64)
    packet[23] = 0x06;  // Protocol (TCP = 6)
    packet[24] = 0xAB; packet[25] = 0xCD;  // Header Checksum
    ...
    // TCP Header (20 bytes)
    packet[34] = 0x1F; packet[35] = 0x90;  // Source Port (8080)
    packet[36] = 0x00; packet[37] = 0x50;  // Destination Port (80)
    packet[38] = 0x12; packet[39] = 0x34;  // Sequence Number
    packet[40] = 0x56; packet[41] = 0x78;
    packet[42] = 0x87; packet[43] = 0x65;  // Acknowledgment Number
    packet[44] = 0x43; packet[45] = 0x21;
    packet[46] = 0x50;  // Data Offset (5) + Reserved (000)
    packet[47] = 0x18;  // Flags (ACK + PSH)
    packet[48] = 0x20; packet[49] = 0x00;  // Window Size (8192)
    packet[50] = 0xEF; packet[51] = 0xBE;  // Checksum
    packet[52] = 0x00; packet[53] = 0x00;  // Urgent Pointer
...
std::array<uint8_t, MAX_PACKET_SIZE> createEthernetARPPacket() {
    ...
    packet[20] = 0x00; packet[21] = 0x01;  // Operation (ARP Request)
    
    // Sender Hardware Address (MAC): 00:11:22:33:44:55
    packet[22] = 0x00; packet[23] = 0x11; packet[24] = 0x22;
    packet[25] = 0x33; packet[26] = 0x44; packet[27] = 0x55;
    
    // Sender Protocol Address (IP): 192.168.1.100
    packet[28] = 192; packet[29] = 168; packet[30] = 1; packet[31] = 100;
    ...
    packet[38] = 192; packet[39] = 168; packet[40] = 1; packet[41] = 1;
    
    return packet;
}

void printProtocolFields(const std::vector<std::unique_ptr<ProtocolModel>>& protocols) {
    for (const auto& protocol : protocols) {
        std::cout << "\n=== " << protocol->getName() << " PROTOCOL ===" << std::endl;
