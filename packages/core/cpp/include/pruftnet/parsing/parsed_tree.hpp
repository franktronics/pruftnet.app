...
struct TreeBuildError {
    ...
    friend constexpr bool operator==(const TreeBuildError&, const TreeBuildError&) = default;
};

template <typename Value> using TreeResult = std::variant<Value, TreeBuildError>;
...
public:
    ...
    [[nodiscard]] std::span<const ParsedContributor> contributors() const noexcept;
    ...
    [[nodiscard]] std::string_view source_name(const ParsedDataSource& source) const noexcept;
    [[nodiscard]] std::span<const std::byte> source_bytes(const ParsedDataSource& source) const noexcept;
    [[nodiscard]] std::span<const ParsedContributor> source_contributors(const ParsedDataSource& source) const noexcept;
    [[nodiscard]] std::span<const std::byte> node_bytes(const ParsedFieldNode& node) const noexcept;
    [[nodiscard]] std::string_view node_text(const ParsedFieldNode& node) const noexcept;
...
public:
    ...
    [[nodiscard]] TreeResult<std::uint32_t> add_none(FieldId field_id, std::uint32_t parent_index,
                                                     DataSourceId data_source_id, std::size_t offset,
                                                     std::size_t length, std::uint32_t flags = ParsedNodeFlagNone);
    [[nodiscard]] TreeResult<std::uint32_t> add_unsigned(FieldId field_id, std::uint32_t parent_index,
                                                         DataSourceId data_source_id, std::size_t offset,
                                                         std::size_t length, std::uint64_t value,
                                                         std::uint32_t flags = ParsedNodeFlagNone);
    [[nodiscard]] TreeResult<std::uint32_t> add_signed(FieldId field_id, std::uint32_t parent_index,
                                                       DataSourceId data_source_id, std::size_t offset,
                                                       std::size_t length, std::int64_t value,
                                                       std::uint32_t flags = ParsedNodeFlagNone);
    [[nodiscard]] TreeResult<std::uint32_t> add_boolean(FieldId field_id, std::uint32_t parent_index,
                                                        DataSourceId data_source_id, std::size_t offset,
                                                        std::size_t length, bool value,
                                                        std::uint32_t flags = ParsedNodeFlagNone);
    [[nodiscard]] TreeResult<std::uint32_t> add_bytes(FieldId field_id, std::uint32_t parent_index,
                                                      DataSourceId data_source_id, std::size_t offset,
    ...
    [[nodiscard]] TreeResult<std::uint32_t> add_string(FieldId field_id, std::uint32_t parent_index,
                                                       ...
                                                       std::uint32_t flags = ParsedNodeFlagNone);
    [[nodiscard]] TreeResult<std::uint32_t> add_generated_text(FieldId field_id, std::uint32_t parent_index,
                                                               DataSourceId data_source_id, std::size_t offset,
                                                               std::size_t length, std::string_view value,
                                                               std::uint32_t flags = ParsedNodeFlagNone);
    [[nodiscard]] TreeResult<std::uint32_t> add_generated_text(FieldId field_id, std::uint32_t parent_index,
    ...
    [[nodiscard]] TreeResult<std::monostate> set_condition(ParseCondition condition) noexcept;
    [[nodiscard]] ParseCondition condition() const noexcept;
    [[nodiscard]] std::span<const ParsedFieldNode> nodes() const noexcept;
    [[nodiscard]] std::span<const ParsedDataSource> data_sources() const noexcept;
...
private:
    [[nodiscard]] TreeResult<std::uint32_t> add_node(FieldId field_id, FieldValueType expected_type,
                                                     ParsedValueTag value_tag, std::uint32_t parent_index,
                                                     DataSourceId data_source_id, std::size_t offset,
                                                     std::size_t length, std::uint64_t value_low,
                                                     std::uint64_t value_high, std::span<const std::byte> byte_value,
                                                     std::string_view text_value, std::uint32_t flags);
    [[nodiscard]] TreeResult<std::uint32_t> validate_node(FieldId field_id, FieldValueType expected_type,
                                                          std::uint32_t parent_index, DataSourceId data_source_id,
                                                          std::size_t offset, std::size_t length,
                                                          std::size_t value_bytes, std::size_t string_bytes,
                                                          std::uint32_t flags);
    [[nodiscard]] TreeBuildError budget_error(TreeBuildErrorCode code, std::size_t requested,
