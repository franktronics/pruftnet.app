...
void strict_upper_bounds_are_rejected() {
    const std::array<void (*)(SnifferInterfaceOptions&), 5> invalidators = {
        [](SnifferInterfaceOptions& value) { value.snaplen = pruftnet::sniffing::kMaxSnapshotLength + 1; },
        [](SnifferInterfaceOptions& value) {
            value.pcap_buffer_size_bytes = pruftnet::sniffing::kMaxPcapBufferSizeBytes + 1;
        },
        [](SnifferInterfaceOptions& value) { value.read_timeout_ms = pruftnet::sniffing::kMaxReadTimeoutMs + 1; },
        [](SnifferInterfaceOptions& value) {
            value.pcap_dispatch_batch_size = pruftnet::sniffing::kMaxPcapDispatchBatchSize + 1;
        },
        [](SnifferInterfaceOptions& value) { value.ring_slots = pruftnet::sniffing::kMaxRingSlots + 1; },
    };
    ...
    const auto error = validate_sniffer_options(too_many, SnifferOptionsValidation{.require_interface_name = false});
    assert(error.has_value());
    assert(error->code == SnifferErrorCode::InvalidOptions);
}
