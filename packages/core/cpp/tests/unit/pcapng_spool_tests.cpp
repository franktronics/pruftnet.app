...
public:
  explicit FaultSink(std::shared_ptr<FaultState> state) : state_(std::move(state)) {}
  ...
  bool flush() noexcept override {
    ++state_->flush_calls;
    if (state_->flush_calls != state_->fail_flush_call) return true;
    error_ = ENOSPC;
    return false;
  }
  ...
  bool close() noexcept override {
    ++state_->close_calls;
    if (state_->close_calls != state_->fail_close_call) return true;
    error_ = EIO;
    return false;
  }
...
public:
  ...
  std::size_t write(std::span<const std::byte> bytes) noexcept override {
    ...
    if (count != bytes.size()) error_ = ENOSPC;
    return count;
  }
...
capture::SpoolSinkFactory fault_factory(const std::shared_ptr<FaultState>& state) {
...
std::filesystem::path test_directory() {
  auto path = std::filesystem::temp_directory_path() / "pruftnet-pcapng-spool-test";
...
void writes_recovers_and_reads_committed_packets() {
  ...
  assert(std::holds_alternative<std::unique_ptr<capture::PcapngSpool>>(created));
  auto spool = std::move(std::get<std::unique_ptr<capture::PcapngSpool>>(created));
  ...
  assert(std::holds_alternative<std::vector<capture::CommittedPacket>>(flushed));
  ...
  assert(lookup.packet->bytes == std::vector<std::byte>(payload.begin(), payload.end()));
  const auto paths = spool->segment_paths();
  assert(paths.size() == 1);
  assert(std::holds_alternative<std::vector<capture::CommittedPacket>>(spool->finalize()));

  const auto recovered = capture::PcapngSpool::recover_segment(paths.front(), false);
  assert(std::holds_alternative<capture::PcapngSpool::RecoveryResult>(recovered));
  const auto& result = std::get<capture::PcapngSpool::RecoveryResult>(recovered);
...
void reports_short_writes_and_flush_failures_without_committing_packets() {
  ...
  auto short_created = capture::PcapngSpool::create(
      options, capture_id, {{0, "fake0", 1, 65535, 9}}, fault_factory(short_state));
  auto short_spool =
      std::move(std::get<std::unique_ptr<capture::PcapngSpool>>(short_created));
  const auto short_error = short_spool->append(metadata(capture_id, 1), payload);
  assert(short_error && short_error->reason == capture::SpoolFailureReason::ShortWrite);
  ...
  auto flush_created = capture::PcapngSpool::create(
      options, capture_id, {{0, "fake0", 1, 65535, 9}}, fault_factory(flush_state));
  auto flush_spool =
...
void publishes_rotation_commits_and_observes_ring_eviction() {
  ...
  assert(std::holds_alternative<std::vector<capture::CommittedPacket>>(first_flush));
  assert(std::get<std::vector<capture::CommittedPacket>>(first_flush).size() == 2);
  assert(!spool->append(metadata(capture_id, 3, 7), payload));
  auto second_flush = spool->flush();
  assert(std::holds_alternative<std::vector<capture::CommittedPacket>>(second_flush));
  assert(std::get<std::vector<capture::CommittedPacket>>(second_flush).size() == 1);
  ...
  assert(spool->lookup({capture_id, 1}).status == capture::PacketSpoolLookupStatus::Evicted);
  assert(spool->lookup({capture_id, 2}).status == capture::PacketSpoolLookupStatus::Found);
  assert(spool->lookup({capture_id, 3}).status == capture::PacketSpoolLookupStatus::Found);
}
...
void disk_full_recovery_preserves_an_independently_readable_prefix() {
  ...
  const std::array payload{std::byte{1}, std::byte{2}, std::byte{3}, std::byte{4}};
  assert(!spool->append(metadata(capture_id, 1), payload));
  assert(std::holds_alternative<std::vector<capture::CommittedPacket>>(spool->flush()));
  ...
  const auto paths = spool->segment_paths();
  assert(std::holds_alternative<std::vector<capture::CommittedPacket>>(spool->finalize()));
  const auto recovered = capture::PcapngSpool::recover_segment(paths.front(), true);
  assert(std::holds_alternative<capture::PcapngSpool::RecoveryResult>(recovered));
  const auto& result = std::get<capture::PcapngSpool::RecoveryResult>(recovered);
