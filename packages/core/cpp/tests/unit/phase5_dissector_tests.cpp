...
void arp_parses_variable_addresses_and_excludes_padding() {
  ...
  assert(node(tree, *registry, "eth.trailer").length == 18);

  auto malformed = bytes;
  malformed[18] = std::byte{100};
  assert(
      parser
          .parse(pruftnet::tests::raw_packet_view(malformed, malformed.size()))
          .condition() == ParseCondition::Malformed);
}

void icmpv4_echo_and_errors_are_bounded() {
  const std::array echo{std::byte{8},    std::byte{0},    std::byte{0x12},
                        std::byte{0x34}, std::byte{0xab}, std::byte{0xcd},
                        std::byte{0},    std::byte{7},    std::byte{'x'}};
  ...
  const std::array unreachable{
      std::byte{3},    std::byte{4},    std::byte{0},
      std::byte{0},    std::byte{0},    std::byte{0},
      std::byte{0x05}, std::byte{0xdc}, std::byte{0x45}};
  const auto error_bytes = ipv4_packet(1, unreachable);
  const auto error_tree = parser.parse(
      pruftnet::tests::raw_packet_view(error_bytes, error_bytes.size()));
...
void ipv6_dispatches_direct_and_bounded_extensions() {
  const std::array udp{std::byte{0x12}, std::byte{0x34}, std::byte{0x00},
                       std::byte{0x36}, std::byte{0x00}, std::byte{0x08},
                       std::byte{0},    std::byte{0}};
  const auto direct = ipv6_packet(17, udp);
  const auto registry = core_registry();
  PacketParser parser(registry);
  const auto direct_tree =
      parser.parse(pruftnet::tests::raw_packet_view(direct, direct.size()));
  ...
  const std::array with_extension{
      std::byte{58},   std::byte{0},    std::byte{0}, std::byte{0},
      std::byte{0},    std::byte{0},    std::byte{0}, std::byte{0},
      std::byte{128},  std::byte{0},    std::byte{0}, std::byte{0},
      std::byte{0x12}, std::byte{0x34}, std::byte{0}, std::byte{1}};
  const auto extended = ipv6_packet(0, with_extension);
  const auto extended_tree =
      parser.parse(pruftnet::tests::raw_packet_view(extended, extended.size()));
...
void ipv6_fragments_do_not_claim_upper_layers() {
  const std::array fragment{
      std::byte{17},   std::byte{0},    std::byte{0},    std::byte{1},
...
void ip_family_dispatch_isolated_and_extension_lengths_are_validated() {
  const std::array icmp_header{std::byte{128}, std::byte{0}, std::byte{0},
                               std::byte{0},   std::byte{0}, std::byte{1},
                               std::byte{0},   std::byte{2}};
  const auto registry = core_registry();
  PacketParser parser(registry);
  const auto wrong_v4 = ipv4_packet(58, icmp_header);
  const auto wrong_v4_tree =
      parser.parse(pruftnet::tests::raw_packet_view(wrong_v4, wrong_v4.size()));
  assert(node_count(wrong_v4_tree, *registry, "icmpv6.message") == 0);
  assert(node(wrong_v4_tree, *registry, "unknown.data").length ==
         icmp_header.size());

  const auto wrong_v6 = ipv6_packet(1, icmp_header);
  const auto wrong_v6_tree =
      parser.parse(pruftnet::tests::raw_packet_view(wrong_v6, wrong_v6.size()));
  assert(node_count(wrong_v6_tree, *registry, "icmp.message") == 0);
  assert(node(wrong_v6_tree, *registry, "unknown.data").length ==
         icmp_header.size());

  const std::array short_ah{std::byte{59}, std::byte{0}, std::byte{0},
                            std::byte{0},  std::byte{0}, std::byte{0},
                            std::byte{0},  std::byte{0}};
  const auto invalid_ah = ipv6_packet(51, short_ah);
  assert(parser
             .parse(pruftnet::tests::raw_packet_view(invalid_ah,
                                                     invalid_ah.size()))
             .condition() == ParseCondition::Malformed);

  const std::array valid_ah{std::byte{59}, std::byte{1}, std::byte{0},
                            std::byte{0},  std::byte{0}, std::byte{0},
                            std::byte{0},  std::byte{0}, std::byte{0},
                            std::byte{0},  std::byte{0}, std::byte{0}};
  const auto ah = ipv6_packet(51, valid_ah);
  assert(parser.parse(pruftnet::tests::raw_packet_view(ah, ah.size()))
             .condition() == ParseCondition::Complete);
}

void unknown_icmp_codes_and_redirect_addresses_preserve_bytes() {
  const std::array unknown_code{std::byte{3}, std::byte{99}, std::byte{0},
                                std::byte{0}, std::byte{1},  std::byte{2},
                                std::byte{3}, std::byte{4},  std::byte{5}};
  const auto registry = core_registry();
  PacketParser parser(registry);
  const auto unknown_bytes = ipv4_packet(1, unknown_code);
  const auto unknown_tree = parser.parse(
      pruftnet::tests::raw_packet_view(unknown_bytes, unknown_bytes.size()));
  ...
  const auto redirect_tree = parser.parse(
      pruftnet::tests::raw_packet_view(redirect_bytes, redirect_bytes.size()));
...
void fragment_offsets_and_dispatch_budgets_are_explicit() {
  const std::array fragment{
      std::byte{17}, std::byte{0}, std::byte{0}, std::byte{8},
  ...
  const auto limited_tree =
      limited.parse(pruftnet::tests::raw_packet_view(bytes, bytes.size()));
...
void selected_icmp_message_families_remain_parseable() {
  ...
  for (const std::uint8_t type :
       {std::uint8_t{0}, std::uint8_t{3}, std::uint8_t{5}, std::uint8_t{8},
    ...
    assert(parser.parse(pruftnet::tests::raw_packet_view(bytes, bytes.size()))
               .condition() == ParseCondition::Complete);
  }
  for (const std::uint8_t type :
  ...
  for (const auto [type, length] :
       {std::pair{std::uint8_t{134}, std::size_t{16}},
...
void arp_and_ipv6_declared_boundaries_are_exhaustive() {
  ...
  assert(
      parser
          .parse(pruftnet::tests::raw_packet_view(oversized, oversized.size()))
          .condition() == ParseCondition::Malformed);

  const std::array invalid_order{
      std::byte{0},  std::byte{0}, std::byte{0}, std::byte{0},
      std::byte{0},  std::byte{0}, std::byte{0}, std::byte{0},
      std::byte{59}, std::byte{0}, std::byte{0}, std::byte{0},
      std::byte{0},  std::byte{0}, std::byte{0}, std::byte{0}};
  const auto ordered = ipv6_packet(60, invalid_order);
  assert(parser.parse(pruftnet::tests::raw_packet_view(ordered, ordered.size()))
             .condition() == ParseCondition::Malformed);
}
...
void icmp_and_nd_truncation_preserve_fixed_prefixes() {
  const auto registry = core_registry();
  PacketParser parser(registry);
  const std::array echo{std::byte{8}, std::byte{0},    std::byte{0},
                        std::byte{0}, std::byte{0x12}, std::byte{0x34},
                        std::byte{0}, std::byte{1},    std::byte{'x'}};
  ...
  for (std::size_t captured = 0; captured < advertisement_bytes.size();
       ++captured) {
    const auto span =
        std::span<const std::byte>(advertisement_bytes.data(), captured);
    assert(parser
               .parse(pruftnet::tests::raw_packet_view(
                   span, advertisement_bytes.size(), 1,
                   pruftnet::sniffing::PacketFlagTruncated))
               .condition() == ParseCondition::Partial);
  }
  ...
  const auto redirect_tree = parser.parse(
      pruftnet::tests::raw_packet_view(redirect_bytes, redirect_bytes.size()));
  assert(node(redirect_tree, *registry, "icmpv6.redirected_packet").length ==
         8);
}
...
void extension_fragment_and_nd_option_matrix_is_bounded() {
  ...
  const std::array two_extensions{
      std::byte{60}, std::byte{0}, std::byte{0}, std::byte{0},  std::byte{0},
      std::byte{0},  std::byte{0}, std::byte{0}, std::byte{17}, std::byte{0},
      std::byte{0},  std::byte{0}, std::byte{0}, std::byte{0},  std::byte{0},
      std::byte{0},  std::byte{0}, std::byte{1}, std::byte{0},  std::byte{2},
      std::byte{0},  std::byte{8}, std::byte{0}, std::byte{0}};
  const auto chained = ipv6_packet(0, two_extensions);
  const auto chained_tree =
      parser.parse(pruftnet::tests::raw_packet_view(chained, chained.size()));
  ...
  assert(node_count(parser.parse(pruftnet::tests::raw_packet_view(
                        atomic, atomic.size())),
                    *registry, "udp.datagram") == 1);

  const std::array misaligned_fragment{
      std::byte{17}, std::byte{0}, std::byte{0}, std::byte{1}, std::byte{0},
  ...
  assert(parser
             .parse(pruftnet::tests::raw_packet_view(unsupported_jumbo,
                                                     unsupported_jumbo.size()))
             .condition() == ParseCondition::Malformed);
  ...
  const auto option_tree = parser.parse(
      pruftnet::tests::raw_packet_view(option_bytes, option_bytes.size()));
  ...
  assert(parser
             .parse(pruftnet::tests::raw_packet_view(overrun_bytes,
                                                     overrun_bytes.size()))
             .condition() == ParseCondition::Malformed);
  ...
  const auto unknown_code_tree = parser.parse(pruftnet::tests::raw_packet_view(
      unknown_code_bytes, unknown_code_bytes.size()));
  ...
  const auto limited_tree =
      limited.parse(pruftnet::tests::raw_packet_view(chained, chained.size()));
  assert(limited_tree.condition() == ParseCondition::ResourceLimit);
