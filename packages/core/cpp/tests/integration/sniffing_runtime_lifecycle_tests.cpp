...
using pruftnet::sniffing::CaptureId;
...
SnifferOptions base_options() { return single_interface_options(); }
...
void stop_before_start_and_repeated_stop_are_safe() {
  ...
  SnifferRuntime runtime(
      base_options(), one_source(std::move(source)),
      SnifferOptionsValidation{.require_interface_name = false},
      [](const auto &, const auto &) {}, EventCallback{});
...
void metadata_and_truncation_are_reported() {
  ...
  assert(retained->source_bytes(retained->data_sources().front()).size() ==
         observed[0].captured_len);
}
...
void configured_capture_identity_is_used() {
  auto options = base_options();
  const CaptureId expected{101, 202};
  options.capture_id = expected;
  auto source = std::make_unique<FakePacketSource>();
  source->packets.push_back(fake_packet(16));
  SnifferRuntime runtime(
      std::move(options), one_source(std::move(source)),
      SnifferOptionsValidation{.require_interface_name = false},
      [](const auto &, const auto &) {}, EventCallback{});

  assert(!runtime.start().has_value());
  runtime.stop();
  assert(runtime.capture_id() == expected);
}
...
void rejected_observations_leave_packet_id_gaps() {
  ...
  assert(stats.packets_observed ==
         stats.capture_queue_accepted + stats.capture_queue_full_drops +
             stats.capture_queue_oversize_drops + stats.invalid_callback_drops);
}
...
int main() {
  ...
  configured_capture_identity_is_used();
