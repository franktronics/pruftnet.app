...
std::size_t padded(std::size_t value) noexcept { return (value + 3U) & ~std::size_t{3U}; }
...
std::uint32_t read_u32(const std::byte* bytes) noexcept {
  ...
  for (unsigned shift = 0; shift < 32; shift += 8)
    value |= std::uint32_t(std::to_integer<unsigned char>(bytes[shift / 8])) << shift;
  return value;
}
...
void append_option(std::vector<std::byte>& output, std::uint16_t code,
  ...
  output.resize(output.size() + padded(value.size()) - value.size(), std::byte{0});
}
...
std::vector<std::byte> section_header(sniffing::CaptureId capture_id) {
  ...
  append_string_option(body, 1, "pruftnet.capture_id=" + capture_hex(capture_id));
...
std::vector<std::byte> interface_description(const SpoolInterface& interface) {
  ...
  append_string_option(body, 1,
                       "pruftnet.interface_id=" + std::to_string(interface.interface_id));
...
public:
...
#endif
    if (file_) std::setvbuf(file_, nullptr, _IOFBF, 1024 * 1024);
    if (!file_) last_error_ = errno;
  }
  ...
  std::size_t write(std::span<const std::byte> bytes) noexcept override {
    ...
    if (written != bytes.size()) last_error_ = errno != 0 ? errno : EIO;
    return written;
  }

  bool flush() noexcept override {
    if (!file_) return false;
    if (std::fflush(file_) == 0) return true;
  ...
  bool close() noexcept override {
    ...
    auto* closing = file_;
    file_ = nullptr;
    if (std::fclose(closing) == 0) return true;
    last_error_ = errno != 0 ? errno : EIO;
    return false;
  }
...
std::optional<std::string_view> option_string(std::span<const std::byte> block,
  ...
  while (offset + 4 <= block.size() - 4) {
    ...
    if (offset + padded(length) > block.size() - 4) return std::nullopt;
    if (code == requested) {
      return std::string_view(reinterpret_cast<const char*>(block.data() + offset), length);
    }
...
bool parse_u64(std::string_view value, std::uint64_t& output, int base = 10) {
  const auto result = std::from_chars(value.data(), value.data() + value.size(), output, base);
  return result.ec == std::errc{} && result.ptr == value.data() + value.size();
}

std::optional<sniffing::CaptureId> parse_capture_comment(std::string_view comment) {
  constexpr std::string_view prefix = "pruftnet.capture_id=";
  if (!comment.starts_with(prefix) || comment.size() != prefix.size() + 32) return std::nullopt;
  sniffing::CaptureId id;
  if (!parse_u64(comment.substr(prefix.size(), 16), id.high, 16) ||
      !parse_u64(comment.substr(prefix.size() + 16), id.low, 16)) return std::nullopt;
  return id;
}
...
PcapngSpool::PcapngSpool(PcapngSpoolOptions options,
    ...
    : options_(std::move(options)), capture_id_(capture_id),
      interfaces_(std::move(interfaces)), sink_factory_(std::move(sink_factory)) {}

PcapngSpool::~PcapngSpool() {
  if (!finalized_) (void)finalize();
  if (!options_.temporary || !finalized_ || preserve_on_destroy_) return;
...
PcapngSpool::create(PcapngSpoolOptions options, sniffing::CaptureId capture_id,
  ...
  if (options.directory.empty()) {
    std::error_code error;
    options.directory = std::filesystem::temp_directory_path(error) / "pruftnet" / "captures";
    if (error)
      return SpoolError{SpoolFailureReason::DirectoryUnavailable,
                        "Unable to resolve the temporary capture directory.", error.value()};
  }
  std::error_code error;
  std::filesystem::create_directories(options.directory, error);
  if (error)
    return SpoolError{SpoolFailureReason::DirectoryUnavailable,
                      "Unable to create pcapng spool directory: " + options.directory.string(),
                      error.value()};
  if (interfaces.empty())
    return SpoolError{SpoolFailureReason::InvalidPacket,
                      "A pcapng spool requires at least one interface.", 0};
  if (std::any_of(interfaces.begin(), interfaces.end(), [](const SpoolInterface& interface) {
        return interface.timestamp_resolution != 6 && interface.timestamp_resolution != 9;
      }))
    return SpoolError{SpoolFailureReason::InvalidPacket,
                      "Pruftnet pcapng spools support microsecond or nanosecond timestamps.", 0};
  if (!sink_factory) sink_factory = make_file_sink;
  auto spool = std::unique_ptr<PcapngSpool>(new PcapngSpool(
      std::move(options), capture_id, std::move(interfaces), std::move(sink_factory)));
  if (auto open_error = spool->open_segment()) return *open_error;
  return spool;
}
...
std::optional<SpoolError> PcapngSpool::open_segment() noexcept {
  try {
    if (options_.max_segments != 0) {
      const auto retained = std::count_if(segments_.begin(), segments_.end(),
                                          [](const Segment& segment) { return !segment.evicted; });
      if (static_cast<std::size_t>(retained) >= options_.max_segments && !options_.ring_mode)
        return SpoolError{SpoolFailureReason::QuotaExceeded,
                          "The configured pcapng segment limit was reached.", 0};
    }
    ...
    if (auto write_error = write_block(shb)) return write_error;
    for (const auto& idb : interface_blocks) {
      if (auto write_error = write_block(idb)) return write_error;
    }
    if (!sink_->flush()) {
      ++flush_failures_;
      return SpoolError{SpoolFailureReason::FlushFailed,
                        "Unable to flush pcapng segment headers.", sink_->last_error()};
    }
    last_flush_ = std::chrono::steady_clock::now();
    while (options_.ring_mode && options_.max_segments != 0 &&
           static_cast<std::size_t>(std::count_if(
               segments_.begin(), segments_.end(),
               [](const Segment& item) { return !item.evicted; })) > options_.max_segments) {
      if (auto eviction_error = evict_oldest_segment()) return eviction_error;
    }
    return std::nullopt;
...
std::optional<SpoolError> PcapngSpool::write_block(std::span<const std::byte> block) noexcept {
  if (!sink_ || segments_.empty())
    return SpoolError{SpoolFailureReason::WriteFailed, "No pcapng spool segment is open.", 0};
  const auto written = sink_->write(block);
  current_offset_ += written;
  bytes_written_ += written;
  bytes_retained_ += written;
  segments_.back().bytes += written;
  if (written == block.size()) return std::nullopt;
  ++write_failures_;
  preserve_on_destroy_ = true;
  return SpoolError{SpoolFailureReason::ShortWrite,
                    "A short write left a recoverable partial pcapng tail.", sink_->last_error()};
}

std::optional<SpoolError> PcapngSpool::rotate_if_needed(std::size_t next_block_bytes) noexcept {
  if (options_.segment_bytes == 0 || segments_.empty() || segments_.back().packets == 0 ||
      current_offset_ + next_block_bytes <= options_.segment_bytes) return std::nullopt;
  const auto flushed = flush_locked();
  if (std::holds_alternative<SpoolError>(flushed)) return std::get<SpoolError>(flushed);
  ...
  if (!sink_->close()) {
    ...
    return SpoolError{SpoolFailureReason::FinalizeFailed,
                      "Unable to close a completed pcapng spool segment.", sink_->last_error()};
  }
  sink_.reset();
  return open_segment();
}
...
std::optional<SpoolError> PcapngSpool::evict_oldest_segment() noexcept {
  ...
  auto candidate = std::find_if(segments_.begin(), segments_.end(), [current](const Segment& segment) {
  ...
  if (error)
    return SpoolError{SpoolFailureReason::WriteFailed,
                      "Unable to evict pcapng segment: " + candidate->path.string(), error.value()};
  candidate->evicted = true;
  bytes_retained_ -= candidate->bytes;
  evicted_bytes_ += candidate->bytes;
  for (auto& entry : index_) {
    if (entry.packet.segment_id == candidate->id && entry.committed && !entry.evicted) {
      entry.evicted = true;
      ++evicted_packets_;
    }
  }
...
std::optional<SpoolError> PcapngSpool::enforce_retention(std::size_t incoming_bytes) noexcept {
  if (options_.max_total_bytes == 0 || bytes_retained_ + incoming_bytes <= options_.max_total_bytes)
    return std::nullopt;
  ...
  while (bytes_retained_ + incoming_bytes > options_.max_total_bytes) {
    if (auto error = evict_oldest_segment()) return error;
  }
  return std::nullopt;
}
...
PcapngSpool::append(const sniffing::PacketMetadata& metadata,
  ...
  if (finalized_)
    return SpoolError{SpoolFailureReason::FinalizeFailed, "The pcapng spool is finalized.", 0};
  if (metadata.key.capture_id != capture_id_ || metadata.captured_len != bytes.size())
    return SpoolError{SpoolFailureReason::InvalidPacket,
                      "Packet metadata does not match the pcapng spool session.", 0};
  const auto interface = std::find_if(interfaces_.begin(), interfaces_.end(), [&](const auto& item) {
  ...
  if (packet_index_.contains(metadata.key.packet_id))
    return SpoolError{SpoolFailureReason::InvalidPacket,
                      "Packet identity is duplicated within the pcapng spool.", 0};
  const auto interface_index = static_cast<std::uint32_t>(std::distance(interfaces_.begin(), interface));
  ...
  if (auto error = write_block(block)) return error;
...
PcapngSpool::flush_locked() noexcept {
  if (pending_indices_.empty()) return std::vector<CommittedPacket>{};
  if (!sink_ || !sink_->flush()) {
    ++flush_failures_;
    preserve_on_destroy_ = true;
    return SpoolError{SpoolFailureReason::FlushFailed,
                      "Unable to flush complete pcapng packet blocks.", sink_ ? sink_->last_error() : 0};
  }
...
std::variant<std::vector<CommittedPacket>, SpoolError> PcapngSpool::flush() noexcept {
  std::lock_guard lock(mutex_);
...
std::variant<std::vector<CommittedPacket>, SpoolError> PcapngSpool::flush_if_due() noexcept {
  std::lock_guard lock(mutex_);
  ...
  const auto due_by_size = options_.flush_bytes == 0 || pending_bytes_ >= options_.flush_bytes;
  const auto due_by_time = options_.flush_interval.count() <= 0 ||
...
std::variant<std::vector<CommittedPacket>, SpoolError> PcapngSpool::finalize() noexcept {
  std::lock_guard lock(mutex_);
  if (finalized_) return std::vector<CommittedPacket>{};
  auto flushed = flush_locked();
  if (std::holds_alternative<SpoolError>(flushed)) return std::get<SpoolError>(flushed);
  auto committed = std::move(deferred_committed_);
  deferred_committed_.clear();
  ...
  if (sink_ && !sink_->close()) {
    ...
    preserve_on_destroy_ = true;
    return SpoolError{SpoolFailureReason::FinalizeFailed,
                      "Unable to finalize the pcapng spool segment.", sink_->last_error()};
  }
  sink_.reset();
  finalized_ = true;
  return committed;
...
PacketSpoolLookup PcapngSpool::lookup(const sniffing::PacketKey& key) const {
  std::lock_guard lock(mutex_);
  if (key.capture_id != capture_id_) return {};
  const auto indexed = packet_index_.find(key.packet_id);
  if (indexed == packet_index_.end()) return {};
  const auto& entry = index_[indexed->second];
  if (!entry.committed) return {};
  if (entry.evicted) return {PacketSpoolLookupStatus::Evicted, std::nullopt};
  const auto segment = std::find_if(segments_.begin(), segments_.end(), [&](const Segment& item) {
    return item.id == entry.packet.segment_id;
  });
  ...
  if (!input) return {PacketSpoolLookupStatus::Corrupt, std::nullopt};
  ...
  input.read(reinterpret_cast<char*>(packet.bytes.data()), static_cast<std::streamsize>(packet.bytes.size()));
  if (!input) return {PacketSpoolLookupStatus::Corrupt, std::nullopt};
...
std::optional<CommittedPacket> PcapngSpool::committed_packet(std::uint64_t ordinal) const {
  std::lock_guard lock(mutex_);
  if (ordinal >= index_.size() || !index_[ordinal].committed) return std::nullopt;
...
PcapngSpoolStats PcapngSpool::stats() const noexcept {
  ...
  stats.segments = std::count_if(segments_.begin(), segments_.end(),
                                 [](const Segment& segment) { return !segment.evicted; });
  return stats;
}
...
std::vector<std::filesystem::path> PcapngSpool::segment_paths() const {
  ...
  for (const auto& segment : segments_)
    if (!segment.evicted) paths.push_back(segment.path);
  return paths;
}
...
PcapngSpool::recover_segment(const std::filesystem::path& path,
  ...
  if (size_error)
    return SpoolError{SpoolFailureReason::OpenFailed,
                      "Unable to stat pcapng segment for recovery.", size_error.value()};
  ...
  while (offset + 12 <= file_size) {
    ...
    if (type == kSectionHeaderBlock) {
      ...
      const auto parsed_capture = comment ? parse_capture_comment(*comment) : std::nullopt;
      if (!parsed_capture || parsed_capture->is_nil())
        return SpoolError{SpoolFailureReason::CorruptData,
                          "The pcapng section lacks a valid capture identity.", 0};
      ...
      if (const auto comment = option_string(block, 16, 1)) {
        ...
        std::uint64_t id = 0;
        if (comment->starts_with(prefix) && parse_u64(comment->substr(prefix.size()), id) &&
            id <= std::numeric_limits<std::uint32_t>::max())
          interface.interface_id = static_cast<std::uint32_t>(id);
      }
      if (const auto name = option_string(block, 16, 2)) interface.name = *name;
      if (const auto resolution = option_string(block, 16, 9);
          resolution && resolution->size() == 1)
        interface.timestamp_resolution = static_cast<std::uint8_t>((*resolution)[0]);
      ...
      if (interface_index >= interfaces.size() || options_offset + 4 > length - 4) break;
      const auto comment = option_string(block, options_offset, 1);
      std::uint64_t packet_id = 0;
      std::uint32_t flags = 0;
      if (!comment || !parse_packet_comment(*comment, packet_id, flags)) break;
      CommittedPacket packet;
      ...
      if (interfaces[interface_index].timestamp_resolution == 6 &&
        ...
        break;
      packet.metadata.timestamp_ns = interfaces[interface_index].timestamp_resolution == 6
          ? timestamp * 1'000
          : timestamp;
  ...
  if (truncate_partial_tail && result.truncated_bytes != 0) {
    ...
    if (resize_error)
      return SpoolError{SpoolFailureReason::FinalizeFailed,
                        "Unable to truncate the invalid pcapng tail.", resize_error.value()};
  }
  return result;
}
...
std::string to_string(SpoolFailureReason reason) {
  switch (reason) {
  case SpoolFailureReason::DirectoryUnavailable: return "DirectoryUnavailable";
  case SpoolFailureReason::OpenFailed: return "OpenFailed";
  case SpoolFailureReason::ShortWrite: return "ShortWrite";
  case SpoolFailureReason::WriteFailed: return "WriteFailed";
  case SpoolFailureReason::FlushFailed: return "FlushFailed";
  case SpoolFailureReason::FinalizeFailed: return "FinalizeFailed";
  case SpoolFailureReason::QuotaExceeded: return "QuotaExceeded";
  case SpoolFailureReason::InvalidPacket: return "InvalidPacket";
  case SpoolFailureReason::CorruptData: return "CorruptData";
  }
  return "Unknown";
}
