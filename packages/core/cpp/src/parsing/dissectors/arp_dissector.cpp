...
DissectionResult dissect_arp(DissectorContext& context, const void* opaque, const PacketView& view,
                              std::uint32_t parent) {
    ...
    const auto logical_length = hardware_length && protocol_length
                                    ? std::size_t{8} + 2U * *hardware_length + 2U * *protocol_length
                                    : std::size_t{8};
    const auto arp_node = context.add_protocol(state.packet, parent, view,
                                               std::min(view.captured_length(), logical_length));
    if (!arp_node || !hardware_length || !protocol_length || context.stopped()) {
        return {};
    }
    ...
    if (!hardware_type || !protocol_type || !operation) {
        return {};
    }
    if (!context.add_unsigned(state.hardware_type, *arp_node, view, 0, 2, *hardware_type) ||
        !context.add_unsigned(state.protocol_type, *arp_node, view, 2, 2, *protocol_type) ||
        !context.add_unsigned(state.hardware_length, *arp_node, view, 4, 1, *hardware_length) ||
        !context.add_unsigned(state.protocol_length, *arp_node, view, 5, 1, *protocol_length) ||
        !context.add_unsigned(state.operation, *arp_node, view, 6, 2, *operation)) {
        return {};
    }
    ...
    const auto add_address = [&](FieldId field, std::size_t length) {
        const auto available = offset < view.captured_length()
                                   ? std::min(length, view.captured_length() - offset)
                                   : std::size_t{0};
        if (available < length) {
            context.mark_partial();
        }
        const bool added = offset > view.captured_length()
                               ? true
