...
DissectionResult dissect_vlan(DissectorContext &context, const void *opaque,
                              const PacketView &view, std::uint32_t parent) {
  const auto &state = *static_cast<const VlanDissectorState *>(opaque);
  const auto vlan_node =
      context.add_protocol(state.tag, parent, view, view.captured_length());
  ...
  if (!context.add_unsigned(state.priority, *vlan_node, view, 0, 2,
                            (*tci >> 13U) & 0x7U) ||
      !context.add_unsigned(state.drop_eligible, *vlan_node, view, 0, 2,
                            (*tci >> 12U) & 0x1U) ||
  ...
  if (!dissect_ethernet_payload(context, *type, *payload.value(), *vlan_node,
                                state.trailer)) {
    return {};
  }
