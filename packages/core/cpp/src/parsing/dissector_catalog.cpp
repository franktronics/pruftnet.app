...
DissectorCatalog::DissectorCatalog(RegistrySnapshotPtr registry)
  ...
  const auto ethernet_handle = registrar.add(dissect_ethernet, ethernet);
  ...
  const auto vlan_handle = registrar.add(dissect_vlan, vlan);
  ...
  registrar.bind_ethertype(0x6558, ethernet_handle);
  ...
  const auto linux_cooked_v1_handle =
      registrar.add(dissect_linux_cooked, linux_cooked_v1);
  const auto linux_cooked_v2_handle =
      registrar.add(dissect_linux_cooked, linux_cooked_v2);
  ...
  const auto null_handle = registrar.add(dissect_null_loopback, null);
  const auto loop_handle = registrar.add(dissect_null_loopback, loop);

  const auto raw = state(RawDissectorState{
      registrar.field("raw.packet"),
  });
  const auto raw_handle = registrar.add(dissect_raw, raw);
  ...
  const auto llc_handle = registrar.add(dissect_llc, llc);
  ...
  const auto snap_information_handle =
      registrar.add(dissect_snap, snap_information);
  const auto snap_non_information_handle =
      registrar.add(dissect_snap, snap_non_information);
  ...
  registrar.bind_ethertype(0x88cc, registrar.add(dissect_lldp, lldp));
  ...
  const auto stp_handle = registrar.add(dissect_stp, stp);
  ...
  for (const auto &definition : core_link_types()) {
    ...
    registrar.bind_dlt(static_cast<std::uint32_t>(definition.value),
                       handle_index);
  }
}
