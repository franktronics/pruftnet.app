...
bool is_link_type_accepted(int link_type,
                           const std::vector<int> &accepted_link_types) {
  return std::find(accepted_link_types.begin(), accepted_link_types.end(),
                   link_type) != accepted_link_types.end();
}

std::string link_type_name(int link_type) {
#if defined(PRUFTNET_HAVE_PCAP_DATALINK_NAME)
  if (const auto *name = pcap_datalink_val_to_name(link_type);
      name != nullptr) {
