...
std::uint64_t packet_timestamp_ns(const pcap_pkthdr &header,
  ...
  return seconds * 1'000'000'000ULL +
         (precision == TimestampPrecision::Nanoseconds ? subsecond
                                                       : subsecond * 1'000ULL);
}

void add_interface_stats(SnifferStatsSnapshot &aggregate,
                         const InterfaceStatsSnapshot &interface_stats) {
  ...
  aggregate.capture_queue_full_drops +=
      interface_stats.capture_queue_full_drops;
  aggregate.capture_queue_oversize_drops +=
      interface_stats.capture_queue_oversize_drops;
  ...
  aggregate.pcap_stats_read_failures +=
      interface_stats.pcap_stats_read_failures;
  ...
  aggregate.capture_queue_capacity_packets +=
      interface_stats.capture_queue_capacity_packets;
  aggregate.capture_queue_capacity_bytes +=
      interface_stats.capture_queue_capacity_bytes;
...
estimated_ring_bytes(std::size_t capacity, std::size_t byte_capacity) noexcept {
  ...
  constexpr auto descriptor_bytes =
      sizeof(PacketMetadata) + sizeof(std::size_t) * 3;
...
struct SnifferRuntime::InterfaceCaptureContext {
  InterfaceCaptureContext(SnifferRuntime &owner,
...
SnifferRuntime::SnifferRuntime(
    ...
    SnifferOptionsValidation validation, PacketCallback packet_callback,
    EventCallback event_callback, capture::SpoolSinkFactory spool_sink_factory)
    : registry_(make_runtime_registry()),
      catalog_(parsing::internal::make_core_dissector_catalog(registry_)),
      options_(std::move(options)), packet_source_count_(packet_sources.size()),
      validation_(validation), packet_callback_(std::move(packet_callback)),
      event_callback_(std::move(event_callback)),
      spool_sink_factory_(std::move(spool_sink_factory)), parser_(catalog_) {
  ...
  for (std::size_t index = 0; index < options_.interfaces.size(); ++index) {
    ...
    interfaces_.push_back(std::make_unique<InterfaceCaptureContext>(
        *this, std::move(interface_options), std::move(source)));
  }
}
...
std::optional<SnifferError> SnifferRuntime::start() {
  ...
  const auto next_capture_id =
      options_.capture_id ? options_.capture_id : make_capture_id();
  if (!next_capture_id.has_value() || next_capture_id->is_nil()) {
    close_sources();
    lock.unlock();
    for (const auto &event : startup_events) {
      emit_event(event);
    ...
    return make_sniffer_error(
        SnifferErrorCode::CaptureIdentityUnavailable, SnifferSeverity::Fatal,
  ...
  for (const auto &context : interfaces_) {
    spool_interfaces.push_back({
        context->options.id,
        context->source ? context->source->source_name()
  ...
  auto created_spool = capture::PcapngSpool::create(
      ...
      spool_sink_factory_);
  if (const auto *spool_error =
          std::get_if<capture::SpoolError>(&created_spool)) {
    close_sources();
    return make_sniffer_error(spool_error_code(spool_error->reason),
                              SnifferSeverity::Fatal, spool_error->message);
  }
  spool_ =
      std::move(std::get<std::unique_ptr<capture::PcapngSpool>>(created_spool));
  ...
  try {
    ...
    for (auto &context : interfaces_) {
      context->capture_thread =
          std::thread(&SnifferRuntime::capture_loop, this, std::ref(*context));
    }
...
void SnifferRuntime::stop() noexcept {
  ...
  const auto called_from_runtime_thread = [&] {
    if ((writer_thread_.joinable() &&
        ...
        (analyzer_thread_.joinable() &&
         analyzer_thread_.get_id() == current_thread)) {
      return true;
    }
    for (const auto &context : interfaces_) {
      if (context->capture_thread.joinable() &&
          context->capture_thread.get_id() == current_thread) {
        return true;
      }
    }
    return false;
...
parsing::RegistryRevision SnifferRuntime::registry_revision() const noexcept {
...
SnifferStatsSnapshot SnifferRuntime::stats() const {
  ...
  for (const auto &context : interfaces_) {
    ...
    const auto ring_byte_capacity =
        context->ring ? context->ring->byte_capacity() : 0;
    auto interface_stats = context->stats.snapshot(
        context->options.id,
        context->source ? context->source->source_name()
                        : context->options.name,
        context->link_type, ring_depth, ring_capacity, ring_bytes,
        ring_byte_capacity,
  ...
  snapshot.packets_available_for_analysis =
      packets_available_for_analysis_.load(std::memory_order_relaxed);
  ...
  snapshot.analysis_evicted_before_analysis =
      analysis_evicted_before_analysis_.load(std::memory_order_relaxed);
  ...
  snapshot.analysis_backlog_packets =
      snapshot.packets_persisted - resolved_analysis;
  snapshot.analysis_backlog_bytes =
      analysis_backlog_bytes_.load(std::memory_order_relaxed);
  snapshot.analysis_errors = analysis_errors_.load(std::memory_order_relaxed);
  snapshot.analysis_resource_limits =
      analysis_resource_limits_.load(std::memory_order_relaxed);
  snapshot.analysis_gap_count =
      analysis_gap_count_.load(std::memory_order_relaxed);
  snapshot.writer_in_flight = writer_in_flight_.load(std::memory_order_relaxed);
  snapshot.terminal_write_losses =
      terminal_write_losses_.load(std::memory_order_relaxed);
  snapshot.writer_thread_running =
      writer_thread_running_.load(std::memory_order_relaxed);
...
std::vector<std::filesystem::path> SnifferRuntime::spool_paths() const {
  const std::lock_guard lock(lifecycle_mutex_);
  return spool_ ? spool_->segment_paths()
                : std::vector<std::filesystem::path>{};
}

std::vector<capture::PcapngSegmentSnapshot>
SnifferRuntime::spool_segments() const {
  std::lock_guard lock(lifecycle_mutex_);
  return spool_ ? spool_->segment_snapshots()
                : std::vector<capture::PcapngSegmentSnapshot>{};
}

std::vector<capture::PcapngSegmentSnapshot>
SnifferRuntime::lease_spool_snapshot() {
  std::lock_guard lock(lifecycle_mutex_);
  return spool_ ? spool_->lease_snapshot()
                : std::vector<capture::PcapngSegmentSnapshot>{};
}

void SnifferRuntime::release_spool_leases(
    std::span<const std::uint64_t> segment_ids) noexcept {
  std::lock_guard lock(lifecycle_mutex_);
  if (spool_)
    spool_->release_leases(segment_ids);
}

void SnifferRuntime::packet_source_callback(
    void *user_data, const pcap_pkthdr &header,
...
std::optional<SnifferError> SnifferRuntime::validate_start_options() const {
  ...
  if (packet_source_count_ != options_.interfaces.size()) {
    return make_sniffer_error(SnifferErrorCode::InvalidOptions,
                              SnifferSeverity::Error,
                              "SnifferRuntime requires exactly one packet "
                              "source per configured interface.");
  }
  ...
  for (const auto &context : interfaces_) {
    if (context->source == nullptr) {
      return make_sniffer_error(
          ...
          "SnifferRuntime requires a packet source for every configured "
          "interface.",
          context->options.name, 0, {}, false, context->options.id);
    }
  }
...
std::optional<SnifferError> SnifferRuntime::open_and_prepare_sources(
  ...
  for (auto &context : interfaces_) {
    ...
    if (std::holds_alternative<SnifferError>(open_result)) {
      return with_interface_context(
          std::get<SnifferError>(std::move(open_result)), *context);
    }
    ...
    if (context->snapshot_length <= 0) {
      return make_sniffer_error(
          ...
          context->source->source_name(), 0, {}, false, context->options.id);
    }

    const auto estimated_bytes = estimated_ring_bytes(
        context->options.ring_slots, context->options.ring_bytes);
  ...
  if (options_.max_total_ring_bytes != 0 &&
      total_ring_bytes > options_.max_total_ring_bytes) {
    std::ostringstream message;
    message << "Packet ring memory budget exceeded. Estimated "
            << total_ring_bytes << " bytes, budget "
            << options_.max_total_ring_bytes << " bytes.";
    return make_sniffer_error(SnifferErrorCode::MemoryBudgetExceeded,
                              SnifferSeverity::Error, message.str());
  }
  ...
  for (auto &context : interfaces_) {
    try {
      ...
      return make_sniffer_error(
          ...
          "Failed to allocate packet ring.", context->source->source_name(), 0,
          {}, false, context->options.id);
    } catch (const std::exception &error) {
      return make_sniffer_error(SnifferErrorCode::InvalidOptions,
...
void SnifferRuntime::join_threads() noexcept {
  ...
  if (capture_threads_joined && !writer_thread_.joinable() &&
      !analyzer_thread_.joinable()) {
...
bool SnifferRuntime::wait_for_start_gate() noexcept {
  ...
  start_gate_condition_.wait(lock, [this] { return start_gate_released_; });
  ...
  try {
    ...
    while (!stop_requested_.load(std::memory_order_acquire)) {
      ...
      const auto result = context.source->dispatch(
          ...
          &SnifferRuntime::packet_source_callback, &context);
      ...
      case PacketSourceDispatchStatus::Error:
        ...
        emit_error(make_sniffer_error(
            ...
            "pcap_dispatch failed.", context.source->source_name(),
            result.pcap_status, result.error_message, false,
            context.options.id));
        break;
      }
...
void SnifferRuntime::publish_committed(
  ...
  packets_available_for_analysis_.fetch_add(packets.size(),
                                            std::memory_order_relaxed);
  std::uint64_t bytes = 0;
  for (const auto &packet : packets)
    bytes += packet.metadata.captured_len;
...
void SnifferRuntime::fail_spool(const capture::SpoolError &error) noexcept {
  if (spool_failed_.exchange(true, std::memory_order_acq_rel))
    return;
  const auto in_flight =
      writer_in_flight_.exchange(0, std::memory_order_acq_rel);
  ...
  try {
    ...
    while (!all_capture_done() || any_ring_has_packets()) {
      ...
      for (std::size_t offset = 0; offset < interfaces_.size(); ++offset) {
        ...
        const auto append_error =
            spool_->append(queued->metadata, queued->bytes);
        context.ring->pop();
        if (append_error) {
    ...
    if (!spool_failed_.load(std::memory_order_acquire)) {
      ...
      if (const auto *error = std::get_if<capture::SpoolError>(&finalized))
        ...
        publish_committed(
            std::get<std::vector<capture::CommittedPacket>>(finalized));
    }
  } catch (const std::exception &error) {
    fail_spool(
        {capture::SpoolFailureReason::WriteFailed,
         std::string("Unhandled exception in pcapng writer: ") + error.what(),
         0});
    account_unwritten_queues();
  } catch (...) {
...
void SnifferRuntime::analyzer_loop() noexcept {
  ...
  try {
    ...
    while (!writer_done_.load(std::memory_order_acquire) ||
      ...
      if (!committed) {
        const auto generation =
            analyzer_wakeup_generation_.load(std::memory_order_acquire);
        if (!writer_done_.load(std::memory_order_acquire) &&
            ordinal >= spool_->committed_count())
          analyzer_wakeup_generation_.wait(generation,
      ...
      if (lookup.status == capture::PacketSpoolLookupStatus::Evicted) {
        ...
        analysis_evicted_before_analysis_.fetch_add(1,
                                                    std::memory_order_relaxed);
...
void SnifferRuntime::handle_packet(InterfaceCaptureContext &context,
  ...
  metadata.key.packet_id =
      next_packet_id_.fetch_add(1, std::memory_order_relaxed);

  if (bytes == nullptr && header.caplen > 0) {
    context.stats.increment_invalid_callback_drops();
    emit_error(make_sniffer_error(
        SnifferErrorCode::InvalidCallbackPayload, SnifferSeverity::Warning,
  ...
  metadata.timestamp_ns =
      packet_timestamp_ns(header, context.source->timestamp_precision());
  ...
  if (pushed != PacketRingPushResult::Accepted) {
    ...
    if (!context.ring_full_reported.exchange(true, std::memory_order_acq_rel)) {
      emit_error(make_sniffer_error(
          SnifferErrorCode::RingFull, SnifferSeverity::Warning,
          pushed == PacketRingPushResult::PacketCapacityReached
              ? "The per-interface capture queue reached its packet capacity; "
                "newest packets are being dropped."
              : "The per-interface capture queue reached its byte capacity; "
                "newest packets are being dropped.",
          context.source->source_name(), 0, {}, true, context.options.id));
    }
    return;
  ...
  context.stats.observe_capture_queue(context.ring->depth(),
                                      context.ring->bytes());
  notify_writer();
}

void SnifferRuntime::update_kernel_stats_if_due(
    InterfaceCaptureContext &context) noexcept {
...
void SnifferRuntime::update_kernel_stats(
    InterfaceCaptureContext &context) noexcept {
  ...
  if (std::holds_alternative<PcapKernelStats>(result)) {
    ...
    context.stats.set_kernel_stats(kernel_stats.recv, kernel_stats.drop,
                                   kernel_stats.ifdrop);
  } else {
    context.stats.increment_pcap_stats_read_failures();
    emit_error(with_interface_context(std::get<SnifferError>(std::move(result)),
                                      context));
  }
}
...
SnifferEvent SnifferRuntime::with_interface_context(
    const SnifferEvent &event, const InterfaceCaptureContext &context) const {
  auto contextual = event;
  if (contextual.interface_id == kAutoInterfaceId ||
      (contextual.interface_id == 0 && context.options.id != 0)) {
    contextual.interface_id = context.options.id;
  }
  if (contextual.interface_name.empty()) {
    contextual.interface_name =
        context.source ? context.source->source_name() : context.options.name;
  }
  return contextual;
}
...
SnifferError SnifferRuntime::with_interface_context(
    SnifferError error, const InterfaceCaptureContext &context) const {
  if (error.interface_id == kAutoInterfaceId ||
      (error.interface_id == 0 && context.options.id != 0)) {
