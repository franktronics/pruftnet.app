...
std::vector<std::byte> ethernet(std::uint16_t type,
...
std::vector<std::byte> ipv6(std::uint8_t next,
                            std::span<const std::byte> payload) {
...
int main() {
  ...
  auto *registry_value =
      std::get_if<pruftnet::parsing::RegistrySnapshot>(&registry_result);
  if (registry_value == nullptr) {
    return 1;
  }
  const auto registry =
      std::make_shared<const pruftnet::parsing::RegistrySnapshot>(
          std::move(*registry_value));
  ...
  auto sample_tree = parser.parse(raw);
  const auto nodes_per_frame = sample_tree.nodes().size();
  parser.recycle(std::move(sample_tree));
  ...
  for (std::size_t index = 0; index < iterations; ++index) {
    ...
    const auto *encoded =
        std::get_if<std::span<const std::byte>>(&encoded_result);
    if (encoded == nullptr) {
      return 1;
    }
    const auto verified_result =
        pruftnet::parsing::verify_packet_tree(*encoded, *registry);
    const auto *verified =
        std::get_if<pruftnet::parsing::VerifiedPacketTreeView>(
            &verified_result);
    if (verified == nullptr) {
      return 1;
    }
  ...
  std::uint64_t mixed_node_count = 0;
  started = std::chrono::steady_clock::now();
  for (std::size_t index = 0; index < iterations; ++index) {
    auto tree = parser.parse(mixed[index % mixed.size()]);
    const auto node_count = tree.nodes().size();
    checksum += node_count;
    mixed_node_count += node_count;
    parser.recycle(std::move(tree));
  }
  const auto mixed_elapsed = std::chrono::steady_clock::now() - started;
  const auto parse_seconds =
      std::chrono::duration<double>(parse_elapsed).count();
  const auto pipeline_seconds =
      std::chrono::duration<double>(pipeline_elapsed).count();
  const auto mixed_seconds =
      std::chrono::duration<double>(mixed_elapsed).count();
  std::cout << "packet_parser.iterations=" << iterations << '\n';
  std::cout << "packet_parser.frame_bytes=" << bytes.size() << '\n';
  std::cout << "packet_parser.nodes_per_frame=" << nodes_per_frame << '\n';
  std::cout << "packet_parser.parse_per_second="
            << static_cast<std::uint64_t>(iterations / parse_seconds) << '\n';
  std::cout << "packet_parser.parse_nodes_per_second="
            << static_cast<std::uint64_t>(iterations * nodes_per_frame /
                                          parse_seconds)
            << '\n';
  std::cout << "packet_parser.pipeline_per_second="
            << static_cast<std::uint64_t>(iterations / pipeline_seconds)
            << '\n';
  std::cout << "packet_parser.pipeline_nodes_per_second="
            << static_cast<std::uint64_t>(iterations * nodes_per_frame /
                                          pipeline_seconds)
            << '\n';
  std::cout << "packet_parser.mixed_per_second="
            << static_cast<std::uint64_t>(iterations / mixed_seconds) << '\n';
  std::cout << "packet_parser.mixed_nodes_per_second="
            << static_cast<std::uint64_t>(mixed_node_count / mixed_seconds)
            << '\n';
